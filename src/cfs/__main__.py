...
import cfs
from cfs.tools import get_runtime_user_information
...
def get_rui(submod):
    ...
    rui = get_runtime_user_information()
    return (
        'This file was generated by cfs {submod}:\n{args}' +
        '\n\n{date}, {user}@{pc}'
    ).format(**rui, args=' '.join(sys.argv), submod=submod)
...
@click.option(
    '--n-components',
    default=cfs.UMAPSimilarity.default_n_components,
    show_default=True,
...
@click.option(
    '--n-neighbors',
    default=cfs.UMAPSimilarity.default_n_neighbors,
    show_default=True,
    type=click.IntRange(min=2),
